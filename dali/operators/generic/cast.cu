...
class CastGPU : public Cast<GPUBackend> {
 ...
 protected:
  ...
  // This is a default anyway, but we need it for Experimental kernel
 ...
 private:
  ...
  // We use experimental kernel for max 8 samples because:
  // 1) params must fit in 4kB
  // 2) sample search is linear (Konrad is working on binary-search version)
  static const int kMaxSamplesExperimental = 8;
  using ExperimentalKernelParams = kernels::ExperimentalCastKernelParams<kMaxSamplesExperimental>;
...
void CastGPU::RunImpl(DeviceWorkspace &ws) {
  ...
  bool useExperimental = input_shape.num_samples() <= kMaxSamplesExperimental;

  kernels::ExperimentalCastKernelParams<kMaxSamplesExperimental> params;
  GpuBlockSetup::BlockDesc *blocks_dev;
  kernels::CastSampleDesc *samples_dev;

  /* Setup for the experimental kernel. */
  if (useExperimental) {
    memcpy(params.samples, samples_.data(), samples_.size() * sizeof(kernels::CastSampleDesc));

    for (int sample_id = 0; sample_id < num_samples; sample_id++) {
      params.sample_sizes[sample_id] = volume(input.tensor_shape(sample_id));
    }

    // Things get ugly and slow here, but it seems enough for prototyping the idea
    ...
    for (int block_id = 0, sample_id = -1; block_id < blocks.size(); block_id++) {
      if (blocks[block_id].sample_idx != sample_id) {
        sample_id++;
        params.first_block[sample_id] = block_id;
      }
    }
  } else {
  ...
  TYPE_SWITCH(output_type_, type2id, OType, CAST_ALLOWED_TYPES, (
    TYPE_SWITCH(itype, type2id, IType, CAST_ALLOWED_TYPES, (
      if (!useExperimental) {
        ...
        kernels::ExperimentalCastKernel<OType, IType, kMaxSamplesExperimental>
           <<<grid_dim, block_dim, 0, ws.stream()>>>(num_samples, kBlockVolumeScale, params);
      }
