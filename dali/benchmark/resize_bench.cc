...
static void ResizeGPUArgs(benchmark::internal::Benchmark *b) {
...
BENCHMARK_DEFINE_F(OperatorBench, ResizeGPU)(benchmark::State& st) {
  int batch_size = st.range(0);
  int H = st.range(1);
  int W = st.range(2);
  float output_size = H / 2;
...
BENCHMARK_REGISTER_F(OperatorBench, ResizeGPU)->Iterations(1000)
